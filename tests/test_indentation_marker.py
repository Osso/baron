...
def check(input_, output):
    for i, j in zip_longest(mark_indentation(input_ + [("ENDMARKER", ""), None]), output + [("ENDMARKER", ""), None]):
        assert i == j
...
def test_empty_line_after_def_2():
    code = "def a():\n    pass\n\ndef b():\n    pass\n"
    parsed = parse(code)
    assert dumps(parsed) == code
    assert len(parsed) == 3
