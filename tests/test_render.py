...
from baron.render import RenderWalker, node_types, nodes_rendering_order, render
...
def test_render_dictionnary_bad_bool_dependency2():
    ...
    assert (
        str(e.value)
        == "Bool keys are only used for dependency, they cannot be rendered. Please set the \"('bool', False, 'other_key')\"'s dependent key in \"bad_bool_dependency2\" node to False"
    )
