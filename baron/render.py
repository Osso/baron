...
def render(node, strict=False):
    ...
    if isinstance(node, list):
        ...
        raise NotImplementedError(
            f"You tried to render a {node.__class__.__name__}. Only list and dicts can be rendered."
        )
...
def render_node(node, strict=False):
    if node["type"] not in nodes_rendering_order:
        raise Exception(
            "There are no defined rules for rendering a node of type '{}', has it been defined in render.py?".format(
                node["type"]
            )
        )
    ...
    for key_type, render_key, dependent in nodes_rendering_order[node["type"]]:
        if not dependent:
            ...
            raise NotImplementedError(
                'Bool keys are only used for dependency, they cannot be rendered. Please set the "{}"\'s dependent key in "{}" node to False'.format(
                    (key_type, render_key, dependent), node["type"]
                )
            )
        elif (
            isinstance(dependent, str)
            and not node.get(dependent)
            or isinstance(dependent, list)
            and not all(node.get(x) for x in dependent)
        ):
            continue
            ...
            try:
                if key_type == "key":
                    assert isinstance(node[render_key], (dict, type(None))), (
                        f"Key '{render_key}' is expected to have type of 'key' (dict/None) but has type of '{type(node[render_key])}' instead"
                    )
                elif key_type == "string":
                    assert isinstance(node[render_key], str), (
                        f"Key '{render_key}' is expected to have type of 'string' but has type of '{type(node[render_key])}' instead"
                    )
                elif key_type in ("list", "formatting"):
                    assert isinstance(node[render_key], list), (
                        f"Key '{render_key}' is expected to have type of 'list' but has type of '{type(node[render_key])}' instead"
                    )
                elif key_type == "constant":
                    pass
                else:
                    raise Exception(
                        f"Invalid key_type '{key_type}', should be one of those: key, string, list, formatting"
                    )
                ...
                sys.stdout.write(
                    "Where node.type == '{}', render_key == '{}' and node ==\n{}\n".format(
        ...
        if key_type in ["key", "string", "list", "formatting"]:
            ...
            raise NotImplementedError('Unknown key type "{}" in "{}" node'.format(key_type, node["type"]))


node_types = {"node", "list", "key", "formatting", "constant", "bool", "string"}
    ...
    * after_leaf: called after encountering a leaf of the FST (can be a constant (like "def" in a function definition) or an actual value like the value a name node)
    ...
    def before(self, key_type, item, render_key):
        if key_type not in node_types:
            raise NotImplementedError(f"Unknown key type: {key_type}")
